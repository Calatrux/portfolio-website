...
const ProjectsSection = () => {
  ...
  const getLogoSrc = (techName: string): string | undefined => {
    const logo = allTechLogos.find(l => l.name.toLowerCase() === techName.toLowerCase());
    return logo?.src;
  };
                ...
                {project.imageCount > 0 ? (
                  <>
                    ...
                    <img 
                      ...
                      key={`${project.imageSlug}-${currentImage}`} // Force re-render for transition if needed, or handle with CSS
                    />
                    {totalImages > 1 && (
                      <>
                        <button 
                ...
                <div className="mt-auto">
                  ...
                  <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
                    {project.technologies.map((tech, techIndex) => {
                      ...
                      return (
                        <div key={techIndex} className="flex items-center bg-neutral-700/80 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-md" title={tech}>
                          {logoSrc ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={logoSrc} alt={tech} className="h-5 w-5 sm:h-6 sm:w-6 mr-1.5 sm:mr-2" />
                          ) : (
                            <span className="h-5 w-5 sm:h-6 sm:w-6 mr-1.5 sm:mr-2 text-xs flex items-center justify-center border border-dashed border-neutral-500 rounded-sm text-neutral-400">?</span>
                          )}
                          <span className="text-xs sm:text-sm text-gray-300">{tech}</span>
                        </div>
                      );
                    })}
                  </div>
